  ...
  grunt.config.merge({
    watch: {
      ...
      tiles: {
        ...
        files: ['<%= opts.build_dir %>/tiles/**/*.{png,jpg,JPG,JPEG,jpeg,svg,gif}'],
        tasks: ['clean:tiles', 'newer:imagemin:tiles', 'newer:eslint:site', 'concat', 'string-replace', 'uglify'] // Need to run JS to update package number
      }
    }
  });
